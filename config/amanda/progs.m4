...
AC_DEFUN([AMANDA_PROG_GROWISOFS],
[
    AC_REQUIRE([AMANDA_INIT_PROGS])
    AC_PATH_PROG(GROWISOFS,growisofs,,$LOCSYSPATH)
    AC_DEFINE_UNQUOTED(GROWISOFS,"$GROWISOFS",
        [Define the location of the growisofs program. ])
])

AC_DEFUN([AMANDA_PROG_DVDRW_MEDIAINFO],
[
    AC_REQUIRE([AMANDA_INIT_PROGS])
    AC_PATH_PROG(DVDRW_MEDIAINFO,dvd+rw-mediainfo,,$LOCSYSPATH)
])
